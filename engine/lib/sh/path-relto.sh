...
usage()
{
  printf 'Usage: %s <path_dest> <path_start>\n' "$SCRIPT"
  printf 'Print the relative path to <path_dest> from <path_start>\n'
}
...
if ! [ "$path_dest" ]; then
  printf '<path_dest> is required\n' >&2
  inputs_valid=
fi

if ! [ "$path_start" ]; then
  printf '<path_start> is required\n' >&2
  inputs_valid=
fi
